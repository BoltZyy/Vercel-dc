...
module.exports = async (req, res) => {
  ...
  if (interaction.type === InteractionType.APPLICATION_COMMAND) {
    ...
    //
    // - /model tidak panggil AI -> aman dieksekusi penuh di sini, langsung
    //   balas hasil final (Type 4), TIDAK perlu deferred.
    // - /tanya butuh panggil AI (lama) -> kita HANYA publish job ke QStash
    //   di sini (operasi cepat, <1 detik), baru setelah publish sukses
    //   kita kirim deferred (Type 5). QStash yang nanti memproses AI lewat
    //   request terpisah & independen ke /api/process-ai.
    ...
    try {
      ...
      if (commandName === 'model') {
        // /model tidak butuh AI/deferred — dijawab langsung Type 4.
      ...
      if (commandName === 'tanya') {
        ...
        try {
          ...
          console.error('[Dispatch] Failed to publish QStash job:', err.message);
          res.status(200).json({
            type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
