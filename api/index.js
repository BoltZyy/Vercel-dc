...
// --- Raw body reader ---
// PENTING: `module.exports.config = { api: { bodyParser: false } }` HANYA
// berlaku untuk Next.js API Routes — project ini pakai @vercel/node polos
// (function biasa di /api/*.js, BUKAN Next.js), jadi config itu TIDAK
// PERNAH benar-benar mencegah Vercel mem-parse body. Ini TERBUKTI lewat
// log produksi: req.body kadang datang sudah berupa OBJECT (bukan Buffer),
// dan fallback lama (re-serialize pakai JSON.stringify) menghasilkan bytes
// yang BEDA dari body asli yang di-sign Discord — bikin verifyKey SELALU
// gagal untuk command tertentu (401, isValid=false), walau DISCORD_PUBLIC_KEY
// benar dan header signature ada.
//
// Fix: JANGAN PERNAH baca dari req.body sama sekali. Baca req SEBAGAI
// STREAM MENTAH secara langsung — ini didukung resmi oleh @vercel/node
// (lihat: https://vercel.com/support/articles/how-do-i-get-the-raw-body-of-a-serverless-function)
// dan tidak bergantung pada config.api yang ternyata tidak berlaku di sini.
