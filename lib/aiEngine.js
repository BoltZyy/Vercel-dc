 ...
 *   Catatan owner tetap disisipkan di belakangnya seperti biasa.
 * @returns {Promise<string>} teks balasan AI
 */
async function askAI({ userMessage, isOwner, history = [], modelOverride, systemPromptOverride }) {
  ...
  const basePrompt = systemPromptOverride || CONFIG.DEFAULT_SYSTEM_PROMPT;
  const systemPrompt = withContextNote(basePrompt, isOwner ? OWNER_VERIFIED_CONTEXT : STANDARD_CONTEXT_NOTE);
