...
/**
 * openP2PAuction — buka lelang P2P baru. Urutan WAJIB:
 *   1. Validasi item luxury dimiliki seller & belum di-lock lelang lain.
 *   2. Validasi saldo cukup buat listing fee (kena diskon Syndicate Boss).
 *   3. Potong fee (100% BURN — TIDAK dikreditkan ke siapa pun).
 *   4. Kunci item (consumeInventoryItem dari inventory aktif + catat di
 *      locked-items supaya tidak muncul dobel di /portfolio /inventory
 *      SELAMA dilelang).
 *   5. Tulis data lelang ke Redis.
 *
 * SEMUA langkah di atas TIDAK atomik satu sama lain (beda dari bid yang
 * pakai Lua) — tapi ini AMAN karena hanya SATU user (seller) yang
 * menyentuh state-nya sendiri di sini, tidak ada race dengan user lain
 * (beda dengan bid yang BANYAK user bisa bid bersamaan).
 */
async function openP2PAuction(sellerId, itemId, startingPrice, durationHours) {
  try {
    const itemDef = getShopItem(itemId);
    if (!itemDef || itemDef.type !== 'luxury') {
      return { ok: false, error: '⚠️ Item tidak valid untuk dilelang. Cuma item kategori luxury yang bisa dilelang.' };
    }

    const cleanPrice = cleanInteger(startingPrice);
    if (cleanPrice <= 0) {
      return { ok: false, error: '⚠️ Harga awal harus lebih dari 0.' };
    }

    const cleanDuration = cleanInteger(durationHours);
    if (cleanDuration <= 0 || cleanDuration > 168) { // maks 7 hari, batas wajar
      return { ok: false, error: '⚠️ Durasi harus antara 1 - 168 jam (7 hari).' };
    }
    ...
    if ((inventory[itemId] || 0) <= 0) {
      return { ok: false, error: `⚠️ Kamu tidak punya ${itemDef.name} untuk dilelang.` };
    }

    // Buff Syndicate Boss: listing fee 1500 -> 1000. Buff Titan
    // Vanguard: tax P2P 10%->5% SELALU (bukan random dalam rentang).
    const { getEquippedBuffTitle } = require('./gachaEngine');
    const equippedTitle = await getEquippedBuffTitle(sellerId);
    const listingFee = equippedTitle === 'SYNDICATE_BOSS' ? P2P_LISTING_FEE_DISCOUNTED : P2P_LISTING_FEE;
    const taxRate = equippedTitle === 'TITAN_VANGUARD'
      ? P2P_TAX_RATE_WITH_BUFF
      : P2P_MIN_TAX_RATE + Math.random() * (P2P_MAX_TAX_RATE - P2P_MIN_TAX_RATE);
    ...
    if (balance < listingFee) {
      return { ok: false, error: `⚠️ Saldo tidak cukup untuk listing fee 💵 ${formatZYC(listingFee)} ZYC. Saldo kamu 💵 ${formatZYC(balance)} ZYC.` };
    }

    // Potong fee DULU (100% burn, tidak dikreditkan ke siapa pun) —
    // sebelum item dikunci, supaya kalau entah kenapa gagal di langkah
    // berikutnya, fee yang sudah terpotong jadi sinyal jelas di log
    // (bukan diam-diam tidak kena apa-apa).
    await adjustBalance(sellerId, -listingFee);

    const consumeResult = await consumeInventoryItem(sellerId, itemId);
    if (!consumeResult.ok) {
      // GAGAL kunci item PADAHAL fee sudah terpotong — kembalikan fee
      // supaya seller tidak dirugikan dari kegagalan sistem.
      await adjustBalance(sellerId, listingFee);
      console.error(`[auctionEngine.openP2PAuction] Gagal consume item ${itemId} untuk seller ${sellerId} setelah fee terpotong — fee sudah dikembalikan.`);
      return { ok: false, error: '⚠️ Gagal mengunci item. Fee sudah dikembalikan, coba lagi.' };
    }

    const auctionId = generateAuctionId();
    const now = Date.now();
    const endsAt = now + cleanDuration * 60 * 60 * 1000;
    ...
    await redis.hset(auctionKey(auctionId), {
      ...
      currentBid: cleanPrice,
      currentBidderId: '',
      endsAt,
      taxRate,
    });
    await redis.sadd(lockedItemsKey(sellerId), `${auctionId}:${itemId}`);

    return { ok: true, auctionId, itemDef, listingFee, taxRate, endsAt };
  } catch (err) {
    console.error(`[auctionEngine.openP2PAuction] Error tak terduga untuk seller ${sellerId}, item ${itemId}:`, err.message);
    return { ok: false, error: '⚠️ Terjadi kesalahan tak terduga saat membuka lelang. Coba lagi ya 🙏' };
  }
}

/**
 * closeAuction — orchestrator penutupan lelang, dipanggil dari job
 * QStash (api/process-auction-close.js). Cabang 2 kemungkinan:
 *   - ADA bidder: transfer item ke bidder, seller dapat (bid - tax),
 *     tax 100% burn. Item TIDAK dikembalikan ke seller (sudah terjual).
 *   - TIDAK ADA bidder: item dikembalikan UTUH ke seller (fee listing
 *     TETAP hangus, sesuai D4 spesifikasi — "No-Bid Refund").
 * SELALU hapus data lelang & entry locked-items di akhir, apa pun
 * hasilnya (sukses atau no-bid).
 */
async function closeAuction(auctionId) {
  try {
    const auction = await getAuction(auctionId);
    if (!auction) {
      console.error(`[auctionEngine.closeAuction] Lelang ${auctionId} tidak ditemukan saat mau ditutup — mungkin sudah ditutup sebelumnya (job duplikat?).`);
      return { ok: false, error: 'Lelang tidak ditemukan (sudah ditutup?).', alreadyClosed: true };
    }

    const redis = getRedis();
    const itemDef = getShopItem(auction.itemId);
    let result;

    if (auction.currentBidderId) {
      // SUKSES — ada bidder tertinggi.
      const taxAmount = cleanInteger(auction.currentBid * auction.taxRate);
      const sellerReceives = auction.currentBid - taxAmount;

      // Transfer item ke bidder (uang bidder SUDAH terpotong sejak
      // placeBid — tidak perlu potong lagi di sini).
      await redis.hincrby(`trading:inventory:${auction.currentBidderId}`, auction.itemId, 1);

      // Bayar seller (bid dikurangi tax, tax 100% burn — TIDAK
      // dikreditkan ke siapa pun, cuma menghilang dari peredaran).
      if (auction.sellerId) {
        await adjustBalance(auction.sellerId, sellerReceives);
      }
      ...
      // NO-BID — kembalikan item UTUH ke seller (P2P saja punya seller;
      // Official auction tanpa bidder cukup dihapus, tidak ada yang
      // "dikembalikan" karena bukan milik siapa pun).
      if (auction.kind === 'P2P' && auction.sellerId) {
        await redis.hincrby(`trading:inventory:${auction.sellerId}`, auction.itemId, 1);
      }
      result = { ok: true, outcome: 'no_bid', itemDef };
    }

    // Bersihkan state lelang & lock item — SELALU dilakukan di akhir,
    // apa pun hasilnya.
    await redis.del(auctionKey(auctionId));
    if (auction.kind === 'P2P' && auction.sellerId) {
      await redis.srem(lockedItemsKey(auction.sellerId), `${auctionId}:${auction.itemId}`);
    }

    return { ...result, auction };
  } catch (err) {
    console.error(`[auctionEngine.closeAuction] Error tak terduga saat menutup lelang ${auctionId}:`, err.message);
    return { ok: false, error: 'Terjadi kesalahan tak terduga saat menutup lelang.' };
  }
}
