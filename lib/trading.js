...
async function setPrice(assetCode, newPrice) {
  ...
  if (!redis) throw new Error('Redis is not configured.');
  const def = getAssetDefinition(assetCode);
  const cleaned = cleanNumber(newPrice);
  // Clamp ke minPrice PER-ASET (bukan 0.01 generik seperti sebelumnya) —
  // ini lapis pertahanan TERAKHIR sebelum tersimpan ke Redis, menutup
  // celah kalau ada caller lain di masa depan yang lupa clamp sendiri
  // sebelum manggil setPrice(). Kalau assetCode entah kenapa tidak
  // dikenal (def null), fallback ke 0.01 supaya tetap tidak pernah
  // tersimpan nilai <= 0.
  const floor = def ? def.minPrice : 0.01;
  const clamped = Math.max(floor, cleaned);
...
async function executeBuy(userId, assetCode, quantity) {
  ...
  // Tracking volume ekosistem (dipakai formula yield dividen Founder
  // Shares, lihat lib/gachaEngine.js) — DIBUNGKUS try/catch TERPISAH
  // dan SENGAJA TIDAK di-await sebagai bagian dari alur utama: transaksi
  // beli SUDAH SELESAI SAH di atas, kegagalan tracking ini (misal Redis
  // sempat error) TIDAK BOLEH membuat seluruh executeBuy melempar error
  // padahal uang & aset user sudah berpindah tangan dengan benar.
  try {
    const { recordEcosystemVolume } = require('./gachaEngine');
    await recordEcosystemVolume(totalCost);
  } catch (err) {
    console.error('[Trading] recordEcosystemVolume failed (non-fatal):', err.message);
  }
...
async function executeSell(userId, assetCode, quantity) {
  ...
  try {
    const { recordEcosystemVolume } = require('./gachaEngine');
    await recordEcosystemVolume(totalGain);
  } catch (err) {
    console.error('[Trading] recordEcosystemVolume failed (non-fatal):', err.message);
  }
...
/**
 * placeOrder — pasang order pending. bypassLimit (default false) HANYA
 * mengecualikan pengecekan maxOrderQuantity di bawah — dipakai dev
 * bypass mode (lib/devHelper.js) lewat command layer. Semua validasi
 * lain (saldo/kepemilikan) TETAP berlaku sama persis, bypass tidak
 * mengubah apa pun selain limit qty ini.
 */
async function placeOrder(userId, assetCode, side, quantity, bypassLimit = false) {
  const redis = getRedis();
  ...
  const qty = cleanInteger(quantity);
  if (qty <= 0) return { ok: false, error: 'Jumlah harus lebih dari 0.' };
  if (!bypassLimit && qty > def.maxOrderQuantity) {
...
async function applyEventToPrice(assetCode, eventData, baseMagnitudePercent) {
  ...
  const rawNewPrice = currentPrice * (1 + eventData.direction * magnitude);

  // Event BEARISH yang cukup keras (terutama pada aset dengan
  // eventSensitivity tinggi seperti VOLT/PLUM) bisa membuat
  // (1 + direction*magnitude) jadi negatif atau mendekati 0 kalau
  // currentPrice sudah rendah — clamp ke minPrice di sini SEBELUM
  // masuk setPrice() (yang juga clamp lagi sebagai lapis pertahanan
  // kedua) supaya tidak pernah ada harga <= 0 yang sempat dihitung.
  const clampedPrice = Math.max(def.minPrice, rawNewPrice);

  return setPrice(assetCode, clampedPrice);
}
...
/* ---------------------------------------------------------------------- */
/* SHOP — inventory (consumable & luxury, permanen) dan cosmetics          */
/* (equipable). Struktur data:                                             */
/*   trading:inventory:{userId}  -> Hash { itemId -> qty }                 */
/*   trading:cosmetics:{userId}  -> Hash { color, title }                  */
/* Item luxury/consumable TIDAK PERNAH bisa dijual balik (money sink murni)*/
/* — makanya di sini cuma ada increment, tidak ada decrement untuk luxury. */
/* ---------------------------------------------------------------------- */

function inventoryKey(userId) {
  return `trading:inventory:${userId}`;
}

function cosmeticsKey(userId) {
  return `trading:cosmetics:${userId}`;
}

/**
 * getInventory — ambil semua item yang dimiliki user sebagai object
 * { ITEM_ID: qty }. Item yang tidak pernah dibeli tidak muncul sebagai
 * key sama sekali (beda dengan getPortfolio yang selalu isi semua aset
 * termasuk 0) — karena daftar item shop bisa terus bertambah, tidak
 * praktis pre-fill semuanya.
 */
async function getInventory(userId) {
  ...
  try {
    ...
    console.error('[Trading] getInventory failed:', err.message);
    return null;
  }
}

/**
 * getCosmetics — ambil kosmetik yang sedang di-equip user, atau object
 * kosong kalau belum pernah beli apa pun. Dipakai langsung oleh
 * /portfolio untuk warna embed & title header.
 */
async function getCosmetics(userId) {
  const redis = getRedis();
  if (!redis) return null;
  try {
    const raw = await redis.hgetall(cosmeticsKey(userId));
    return raw || {};
  } catch (err) {
    console.error('[Trading] getCosmetics failed:', err.message);
    return {};
  }
}

/**
 * executeShopPurchase — orchestrator SATU-SATUNYA jalur resmi checkout
 * shop. Menggabungkan: baca saldo/loan/inventory dari Redis, validasi
 * murni via validateShopPurchase() (lib/shopItems.js), lalu eksekusi
 * potong saldo + tulis item — SEMUA atau TIDAK SAMA SEKALI (validasi
 * selalu selesai dulu SEBELUM baris tulis pertama dieksekusi).
 *
 * TIDAK memanggil checkAndHandleOverdueLoan() di sini — itu tetap
 * tanggung jawab command layer (konsisten dengan buysell.js/posisi.js/
 * debt.js yang semua memanggilnya sendiri di awal handler).
 */
async function executeShopPurchase(userId, itemId) {
  const { getShopItem, validateShopPurchase } = require('./shopItems');
  ...
  const [cash, loan, inventory] = await Promise.all([
    getBalance(userId),
    getLoan(userId),
    getInventory(userId),
  ]);

  if (cash === null || inventory === null) {
    return { ok: false, error: 'Redis tidak dikonfigurasi.' };
  }

  const loanAmount = loan ? cleanInteger(loan.amount) : 0;
  const validation = validateShopPurchase(item, { cash, loanAmount, inventory });
  if (!validation.ok) {
    return { ok: false, error: validation.reason };
  }

  const newBalance = await adjustBalance(userId, -item.price);

  if (item.type === 'equipable') {
    await redis.hset(cosmeticsKey(userId), { [item.cosmeticSlot]: item.cosmeticValue });
    return { ok: true, newBalance, item, equipped: true };
  }

  // consumable & luxury: sama-sama nambah unit di inventory. Bedanya
  // consumable BOLEH berkurang lagi nanti saat dipakai (mis. /leak),
  // luxury TIDAK PERNAH berkurang (tidak ada fungsi decrement untuk itu).
  const newQty = await redis.hincrby(inventoryKey(userId), item.id, 1);
  return { ok: true, newBalance, item, newQuantity: cleanInteger(newQty) };
}

/**
 * consumeInventoryItem — kurangi 1 unit item consumable (mis. dipakai
 * /leak). TIDAK dipakai untuk item luxury (money sink permanen, tidak
 * ada jalur pengurangannya sama sekali). Menghapus key hash kalau
 * habis, konsisten dengan pola adjustAssetQuantity di portfolio.
 */
async function consumeInventoryItem(userId, itemId) {
  const redis = getRedis();
  if (!redis) throw new Error('Redis is not configured.');
  const owned = await getInventory(userId);
  const currentQty = owned?.[itemId] || 0;
  if (currentQty <= 0) {
    return { ok: false, error: 'Item tidak dimiliki.' };
  }
  const newQty = await redis.hincrby(inventoryKey(userId), itemId, -1);
  const cleaned = cleanInteger(newQty);
  if (cleaned <= 0) {
    await redis.hdel(inventoryKey(userId), itemId);
  }
  return { ok: true, newQuantity: Math.max(cleaned, 0) };
}
