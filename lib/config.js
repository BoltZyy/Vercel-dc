...
const CONFIG = {
  ...
  // ===== Sistem Trading Fiktif (ZYC) =====
  // Modal awal semua user, dalam ZYC.
  STARTING_BALANCE: Number(process.env.STARTING_BALANCE || 10000),
  // Tenor pinjaman dalam hari — sama untuk semua user & semua pinjaman.
  // Lewat dari ini dianggap nunggak, dicek saat user pakai command trading.
  LOAN_DUE_DAYS: Number(process.env.LOAN_DUE_DAYS || 7),
  // Channel Discord untuk pengumuman event pasar (T-1 menit sebelum event
  // terjadi). Fallback ke LOG_CHANNEL_ID kalau kosong, skip total kalau
  // keduanya kosong (event tetap terjadi, cuma tanpa pengumuman).
  MARKET_ANNOUNCEMENT_CHANNEL_ID: process.env.MARKET_ANNOUNCEMENT_CHANNEL_ID || '',
  // Persentase (0-1) kemungkinan event pasar terpicu OTOMATIS tiap kali
  // update harga berkala jalan (di luar trigger manual Owner via
  // /market-event). Default 0.15 = 15%.
  RANDOM_EVENT_CHANCE: Number(process.env.RANDOM_EVENT_CHANCE || 0.15),
};
