...
async function verifyAndParseQStashRequest(req, logPrefix) {
  ...
  // QStash Schedule (cron) memanggil endpoint TANPA body (rawBody kosong /
  // hanya whitespace) — ini valid dan BUKAN error, beda dengan job biasa
  // (publishJob) yang selalu mengirim JSON payload. Fallback ke {} supaya
  // endpoint seperti process-price-update (yang tidak butuh payload) tidak
  // gagal di JSON.parse('').
