...
const { getConversation, appendConversation, recordUsage, saveRetryJob } = require('./redis');
...
async function processAiJob({ token, channelId, userId, userMessage, isOwner, jobType = 'tanya', extra = {} }) {
  try {
    ...
    const { text: replyText, usage, modelUsed } = result;
    console.log(`[processAiJob:${jobType}] model=${modelUsed} userId=${userId} channelId=${channelId}`);
    ...
    // Simpan payload job asli supaya bisa diulang lewat tombol Retry.
    // Kalau Redis tidak dikonfigurasi (saveRetryJob return null), pesan
    // error dikirim TANPA tombol — retry cuma mengulang command manual.
    const jobId = await saveRetryJob({ token, channelId, userId, userMessage, isOwner, jobType, extra });
    ...
    await editOriginalInteractionResponse(token, errorPayload).catch(() => {});
