...
function crystallizeSavingsInterest(account, now = Date.now()) {
  ...
  const { interest } = calculateInterest({ dailyRate: rate }, account.balance, account.lastAccrualAt, now);
  return {
...
async function getSavingsStatus(userId) {
  ...
  const rate = await getSavingsRate(userId);
  const { interest: pendingInterest } = calculateInterest({ dailyRate: rate }, existing.balance, existing.lastAccrualAt, Date.now());
