 ...
 *
 * bypassStamina — dipakai dev bypass mode (lib/devHelper.js). Kalau
 * true, cek stamina DAN consumeStamina() sama-sama DILEWATI SEPENUHNYA
 * (bukan cuma dilonggarkan) — supaya stamina user tidak pernah jadi
 * negatif akibat testing, dan tetap sama persis seperti sebelum
 * bypass dipakai kalau nanti dimatikan lagi. Default false supaya
 * SEMUA caller lama tetap berjalan identik seperti sebelumnya.
 */
async function executeWork(userId, jobId, bypassStamina = false) {
  const job = getJob(jobId);
  if (!job) return { ok: false, error: '⚠️ Pekerjaan tidak dikenal. Cek pilihan yang tersedia di `/work`.' };

  let remainingStamina;

  if (bypassStamina) {
    // Sengaja TIDAK memanggil getStaminaState()/consumeStamina() di
    // jalur ini — stamina asli user tidak tersentuh sama sekali.
    remainingStamina = MAX_STAMINA;
  } else {
    ...
    remainingStamina = Math.max(0, newCharges);
  }
  ...
  return { ok: true, job, amount, outcome, isSuccess, remainingStamina };
}
