 ...
 * @param {number} [params.delaySeconds] - kalau diisi, QStash menunda
 *   pengiriman job selama sekian detik (dipakai /remind). Kalau kosong,
 *   job dikirim secepatnya seperti biasa (dipakai /tanya, /status, dll).
 */
async function publishJob({ endpointPath, payload, delaySeconds }) {
...
/**
 * publishRemindJob — shortcut publishJob khusus ke /api/process-remind,
 * dengan delay sesuai waktu yang diminta user (bisa 0 kalau reminder-nya
 * langsung, walau realistisnya selalu >0 untuk /remind).
 */
async function publishRemindJob(payload, delaySeconds) {
  return publishJob({ endpointPath: '/api/process-remind', payload, delaySeconds });
}

module.exports = { getQStash, publishJob, publishAiJob, publishStatusJob, publishRemindJob };
