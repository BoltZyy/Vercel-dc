...
const { getAllPrices, getActiveEvent, setPrice, EVENT_TYPES, cleanNumber, formatZYC } = require('../../trading');
 ...
 *
 * Harga aset BOLEH desimal (cleanNumber, bukan cleanInteger) — cuma
 * saldo/unit aset yang wajib Integer bulat, diatur di trading.js.
 * ========================================================================= */
...
async function handleMarket(interaction, res) {
  ...
  const lines = ASSET_CODES.map((code) => {
    ...
    return `${def.emoji} **${code}** (${def.name}) — 💵 ${formatZYC(prices[code])} ZYC`;
  });
 ...
 * menjalankan triggerMarketEventFlow() — konsisten dengan pola /status
 * dan /export, karena triggerMarketEventFlow melakukan network call
 * ganda yang berisiko >3 detik kalau dijalankan langsung.
 */
...
/**
 * handleMarketSetPrice — Owner paksa harga aset. Instan, tidak perlu
 * deferred. Harga disanitasi cleanNumber (boleh desimal), BUKAN
 * cleanInteger — cuma saldo/unit yang wajib bulat.
 */
async function handleMarketSetPrice(interaction, res) {
  ...
  const newPrice = cleanNumber(hargaOpt?.value);
  ...
  try {
    ...
    res.status(200).json({
      type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
      data: { content: `✅ Harga ${assetCode.toUpperCase()} dipaksa jadi 💵 ${formatZYC(applied)} ZYC.` },
    });
  } catch (err) {
    res.status(200).json({
      type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
