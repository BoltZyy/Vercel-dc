...
async function handlePortfolio(interaction, res) {
  ...
  // Resolusi target: opsional, default ke pemanggil sendiri. Kalau target
  // diisi dan BEDA dari pemanggil, kita HANYA membaca datanya — TIDAK
  // pernah memanggil checkAndHandleOverdueLoan untuk target. Kalau ini
  // dibiarkan longgar, siapa saja bisa memicu sitaan aset ORANG LAIN
  // cuma dengan mengintip portfolio-nya, yang jelas celah abuse.
  const options = interaction.data?.options || [];
  const targetOpt = options.find((o) => o.name === 'target');
  const resolvedUsers = interaction.data?.resolved?.users || {};

  const userId = targetOpt?.value || invokerId;
  const isViewingOther = targetOpt?.value && targetOpt.value !== invokerId;
  const targetDiscordUser = isViewingOther ? resolvedUsers[targetOpt.value] : null;

  // checkAndHandleOverdueLoan HANYA untuk pemanggil sendiri, tidak pernah
  // untuk target orang lain.
  const overdueResult = isViewingOther
    ? { seized: false }
  ...
  // sini (consumable seperti INSIDER_PASS tidak relevan buat "flexing").
  ...
  const embedTitle = isViewingOther
    ? `💼 Portfolio ${targetDiscordUser?.global_name || targetDiscordUser?.username || 'User'}`
    : cosmetics?.title
      ? `💼 Portfolio — ${cosmetics.title}`
      : '💼 Portfolio Kamu';
  ...
  const embed = { title: embedTitle, color: safeColor, fields };
  if (isViewingOther && targetDiscordUser?.avatar) {
    const ext = targetDiscordUser.avatar.startsWith('a_') ? 'gif' : 'png';
    embed.thumbnail = { url: `https://cdn.discordapp.com/avatars/${targetOpt.value}/${targetDiscordUser.avatar}.${ext}?size=256` };
  }
  const embeds = [embed];
