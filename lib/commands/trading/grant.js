...
const { adjustBalance, adjustAssetQuantity, cleanInteger, formatZYC } = require('../../trading');
 ...
 *
 * Parameter 'jumlah' bertipe NUMBER (type: 10) di Discord, artinya BOLEH
 * desimal dari sisi user — cleanInteger() WAJIB dipanggil SEBELUM
 * validasi, karena typeof check saja tidak cukup (500.7 tetap typeof
 * 'number', lolos validasi lama, tapi bikin hincrby di Redis error/aneh
 * karena Redis hash increment tidak mendukung float).
 * ========================================================================= */
...
async function handleGrant(interaction, res) {
  ...
  if (amount === 0) {
    res.status(200).json({
      type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
      data: { content: '⚠️ Jumlah harus angka bulat, tidak boleh 0 (boleh negatif buat mengurangi).' },
    });
    return;
  }
  ...
  try {
    if (tipe === 'cash') {
      ...
      res.status(200).json({
        type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
        data: {
          content: `✅ 💵 ${formatZYC(Math.abs(amount))} ZYC ${verb} saldo <@${targetId}>.\nSaldo sekarang: 💵 ${formatZYC(newBalance)} ZYC`,
        },
      });
      return;
    }
