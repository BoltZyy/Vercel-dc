 ...
 *
 * Dev bypass mode (lib/devHelper.js): kalau aktif, cek DAN pemakaian
 * stamina dilewati sepenuhnya lewat parameter executeWork(..., true).
 * ========================================================================= */
...
async function handleWork(interaction, res) {
  ...
  // Dev bypass: abaikan blokir overdue loan sepenuhnya — jangan panggil
  // checkAndHandleOverdueLoan() sama sekali, supaya tidak ada sitaan
  // aset yang ke-trigger saat testing.
  if (!bypassed) {
  ...
  const result = await executeWork(userId, job.id, bypassed);
  if (!result.ok) {
    res.status(200).json({
      type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
