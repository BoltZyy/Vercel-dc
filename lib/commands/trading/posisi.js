 ...
 *
 * Dev bypass mode (lib/devHelper.js): kalau aktif, overdue loan check
 * dilewati SEPENUHNYA (tidak dipanggil), dan placeOrder() dipanggil
 * dengan bypassLimit=true (HANYA melewati maxOrderQuantity — validasi
 * saldo/kepemilikan di dalam placeOrder tetap berlaku normal).
 * ========================================================================= */
...
async function handlePosisi(interaction, res) {
  ...
  if (buyOpt) {
    await handlePlaceOrder(interaction, res, userId, buyOpt, 'buy', bypassed);
    return;
  }
  if (sellOpt) {
    await handlePlaceOrder(interaction, res, userId, sellOpt, 'sell', bypassed);
    return;
  }
  if (batalOpt) {
...
async function handlePlaceOrder(interaction, res, userId, subOpt, side, bypassed = false) {
  ...
  const result = await placeOrder(userId, assetCode, side, quantity, bypassed);
  if (!result.ok) {
    res.status(200).json({
      type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
