...
async function handleBlock(interaction, res) {
  ...
  try {
    ...
    await logAuditEvent('block', {
      userId: targetId,
      moderatorId: getInvokerId(interaction),
      reason: reasonOpt?.value || 'No reason provided',
    });
    res.status(200).json({
      type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
...
async function handleUnblock(interaction, res) {
  ...
  try {
    ...
    if (wasBlocked) {
      await logAuditEvent('unblock', { userId: targetId, moderatorId: getInvokerId(interaction) });
    }
    res.status(200).json({
      type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
      data: {
