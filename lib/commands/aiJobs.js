...
const { saveRetryJob } = require('../redis');
...
async function queueGenericAiJob(interaction, res, { jobType, userMessage, extra }) {
  ...
  try {
    ...
    // Simpan job untuk tombol Retry, sama seperti kegagalan AI di
    // processAiJob — kalau Redis tidak dikonfigurasi, pesan tanpa tombol.
    const jobId = await saveRetryJob({
      token: interaction.token,
      channelId,
      userId: invokerId,
      userMessage,
    ...
    const errorPayload = { content: '⚠️ Gagal menjadwalkan proses AI. Coba lagi beberapa saat ya 🙏' };
    if (jobId) {
      errorPayload.components = [
        {
          type: 1,
          components: [{ type: 2, style: 1, label: '🔄 Coba Lagi', custom_id: `retry:${jobId}` }],
        },
      ];
    }
