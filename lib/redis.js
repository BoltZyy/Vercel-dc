...
/* =========================================================================
 * 4b. AUDIT LOG — log gabungan untuk /audit-log: block, unblock, say,
 * warn, dan error, semua masuk 1 timeline kronologis. Key terpisah dari
 * saylog di atas (yang tetap dipertahankan sebagai backup lama /say
 * secara spesifik) — auditlog adalah pandangan gabungan lintas jenis.
 * Key: auditlog -> list of JSON entries (push ke kiri, terbaru duluan)
 * ========================================================================= */

const AUDIT_LOG_KEY = 'auditlog';
const AUDIT_LOG_MAX_ENTRIES = 500;
const AUDIT_LOG_TTL_SECONDS = 30 * 24 * 3600; // 30 hari, sama dengan say log

/**
 * logAuditEvent — catat 1 event ke audit log gabungan. Best-effort,
 * tidak pernah throw — kegagalan logging tidak boleh menggagalkan
 * command yang memicunya.
 * @param {string} type - 'block' | 'unblock' | 'say' | 'warn' | 'error'
 * @param {Object} data - detail spesifik event (userId, moderatorId, alasan, dll)
 */
async function logAuditEvent(type, data) {
  const redis = getRedis();
  if (!redis) return;
  try {
    const entry = JSON.stringify({ type, ...data, at: new Date().toISOString() });
    await redis.lpush(AUDIT_LOG_KEY, entry);
    await redis.ltrim(AUDIT_LOG_KEY, 0, AUDIT_LOG_MAX_ENTRIES - 1);
    await redis.expire(AUDIT_LOG_KEY, AUDIT_LOG_TTL_SECONDS);
  } catch (err) {
    console.error('[Redis] logAuditEvent failed:', err.message);
  }
}

/**
 * getAuditLog — ambil entri audit log terbaru, opsional filter by type.
 */
async function getAuditLog({ limit = 20, type } = {}) {
  const redis = getRedis();
  if (!redis) return [];
  try {
    // Ambil lebih banyak dari limit kalau ada filter type, supaya hasil
    // akhir setelah filter tetap sekitar `limit` entri.
    const fetchCount = type ? Math.min(AUDIT_LOG_MAX_ENTRIES, limit * 5) : limit;
    const raw = await redis.lrange(AUDIT_LOG_KEY, 0, fetchCount - 1);
    const parsed = raw
      .map((item) => {
        try {
          return typeof item === 'string' ? JSON.parse(item) : item;
      ...
      .filter(Boolean);
    const filtered = type ? parsed.filter((e) => e.type === type) : parsed;
    return filtered.slice(0, limit);
  } catch (err) {
    console.error('[Redis] getAuditLog failed:', err.message);
    return [];
  }
}
...
/* =========================================================================
 * 7. PERSONALITY OVERRIDE — ganti gaya kepribadian bot on-the-fly tanpa
 * redeploy, pola sama persis dengan Model Override di atas.
 * Key: active_personality -> string system prompt tambahan, atau tidak
 * ada (pakai default ENV SYSTEM_PROMPT / DEFAULT_SYSTEM_PROMPT).
 * ========================================================================= */

const PERSONALITY_OVERRIDE_KEY = 'active_personality';

async function setPersonalityOverride(personalityText) {
  const redis = getRedis();
  if (!redis) throw new Error('Redis is not configured.');
  await redis.set(PERSONALITY_OVERRIDE_KEY, personalityText);
}

async function getPersonalityOverride() {
  const redis = getRedis();
  if (!redis) return null;
  try {
    const val = await redis.get(PERSONALITY_OVERRIDE_KEY);
    return val || null;
  } catch (err) {
    console.error('[Redis] getPersonalityOverride failed:', err.message);
    return null;
  }
}

async function clearPersonalityOverride() {
  const redis = getRedis();
  if (!redis) throw new Error('Redis is not configured.');
  await redis.del(PERSONALITY_OVERRIDE_KEY);
}

/* =========================================================================
 * 8. RETRY JOB STORAGE — dipakai Retry button pada /tanya, /translate,
 * /ringkas yang gagal. Discord custom_id maksimal 100 karakter, tidak
 * cukup untuk simpan payload job lengkap (userMessage bisa panjang) —
 * jadi payload asli disimpan di Redis dengan ID pendek, custom_id cuma
 * berisi referensi ID itu (format: "retry:{jobId}").
 * Key: retryjob:{jobId} -> JSON payload job asli. TTL pendek (15 menit,
 * sama dengan masa berlaku interaction token Discord) — kalau lewat itu,
 * mengulang command lama sudah tidak relevan lagi.
 * ========================================================================= */

const RETRY_JOB_TTL_SECONDS = 15 * 60; // 15 menit, sama dengan masa berlaku interaction token
...
/**
 * saveRetryJob — simpan payload job yang gagal, supaya bisa diulang lewat
 * tombol Retry. Return jobId (dipakai sebagai bagian custom_id tombol),
 * atau null kalau Redis tidak dikonfigurasi (Retry button tidak akan
 * ditampilkan sama sekali kalau ini gagal — lihat pemanggil di commands.js).
 */
async function saveRetryJob(payload) {
  const redis = getRedis();
  if (!redis) return null;
  try {
    const jobId = generateJobId();
    await redis.set(`retryjob:${jobId}`, payload, { ex: RETRY_JOB_TTL_SECONDS });
    return jobId;
  } catch (err) {
    console.error('[Redis] saveRetryJob failed:', err.message);
    return null;
  }
}

/**
 * getRetryJob — ambil payload job asli berdasarkan jobId dari custom_id
 * tombol. Tidak dihapus setelah dibaca — user boleh klik retry lebih
 * dari sekali selama TTL belum habis.
 */
async function getRetryJob(jobId) {
  const redis = getRedis();
  if (!redis) return null;
  try {
    const data = await redis.get(`retryjob:${jobId}`);
    return data || null;
  } catch (err) {
    console.error('[Redis] getRetryJob failed:', err.message);
    return null;
  }
}
